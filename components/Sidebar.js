...
export default function Sidebar({currentUser}){
  ...
  const visibleGroups=useMemo(()=>groups.map(g=>({...g,items:g.items.filter(([href])=>canAccessPath(role,href))})).filter(g=>g.items.length),[role]);
  ...
  return <aside className="sidebar">
    ...
    <nav className="sidebar-nav">
      ...
      {lastGroups.length?<div className="sidebar-section-separator"/>:null}
      {lastGroups.map(group=><SidebarGroup key={group.key} {...{group,pathname,openGroups,toggleGroup}}/>)}
      {canAccessPath(role,'/weekly-report')?<><div className="sidebar-section-separator"/><Link href="/weekly-report" className={`nav-item ${isPathActive(pathname,'/weekly-report')?'active':''}`}><span className="nav-visual slate"><NavIcon type="report"/></span><span>Weekly Report</span></Link></>:null}
    </nav>
    <div className="sidebar-note"><strong>{currentUser?.role==='admin'?'Full Access':'Role-based Access'}</strong><span>Live sustainability monitoring with company-level overview and role-aware navigation.</span></div>
  </aside>
}
