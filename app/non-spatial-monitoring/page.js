...
export default function NonSpatialMonitoring(){
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.panel}>
      ...
      <div className={styles.source}>Sumber otomatis: GDELT DOC 2.0. Positive/negative menggunakan indikasi machine tone; hasil tetap perlu review manusia. Rentang otomatis maksimum sekitar 3 bulan per pencarian.</div>
    </section>
    {error&&<div className={`${styles.notice} ${styles.error}`}><strong>Monitoring gagal: </strong>{error}</div>}
    {data&&<>
      <div className={`${styles.notice} ${styles.info}`}>Generated untuk <strong>{data.meta.entity}</strong> · {data.meta.start} s.d. {data.meta.end} · {data.totals.total} berita terindikasi positif/negatif.</div>
