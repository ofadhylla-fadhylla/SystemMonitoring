...
export default function GrievanceDetail() {
  ...
  async function loadCase() {
    ...
    const lookupValue = decodeURIComponent(String(rawCaseId || '')).trim();
    const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(lookupValue);

    let caseQuery = supabase
      .from('grievances')
      .select('*');

    // Support both public Case ID (GRV-0001) and the Supabase UUID.
    // This keeps old/new links compatible and prevents a valid case from showing "Case not found".
    caseQuery = isUuid
      ? caseQuery.eq('id', lookupValue)
      : caseQuery.eq('case_id', lookupValue.toUpperCase());
